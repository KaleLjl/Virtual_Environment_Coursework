...
public class AvatarSys : MonoBehaviour {
    ...
    void InstantiateAvatar() {
        ...
        // Set the initial transform for the avatar with physics consideration:
        // Position: Slightly above ground to prevent physics glitches
        // Rotation: Reset to default
        // Scale: Maintain proper proportions
        target.transform.position = new Vector3(2, 1.5f, 2); // Lower Y position
        target.transform.rotation = Quaternion.identity;
        target.transform.localScale = new Vector3(1, 1, 1);
        
        // Ensure the avatar has a Rigidbody component
        Rigidbody rb = target.GetComponent<Rigidbody>();
        if (rb == null) {
            rb = target.AddComponent<Rigidbody>();
        }
        
        // Configure Rigidbody properties for stable physics
        rb.useGravity = true;
        rb.isKinematic = false;
        rb.mass = 70f; // Set appropriate mass for a human character
        rb.linearDamping = 1f;  // Add air resistance
        rb.angularDamping = 0.05f;
        // Freeze rotation and position on specific axes
